...
export default function AdminCategories() {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: FormData }) => {
      const res = await fetch(`/api/categories/${id}`, { method: "PUT", body: data });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["categories"] });
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">Product categories like PP, PE, PVC, etc.</p>
            </div>
          </div>
            ...
            <DialogContent className="max-w-md">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g., Polypropylene (PP)" required />
                </div>
                <div>
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-center py-8 text-gray-500">No categories yet. Add your first category!</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
