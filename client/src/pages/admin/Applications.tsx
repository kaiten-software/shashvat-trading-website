...
import { Plus, Pencil, Trash2, Factory, ArrowLeft } from "lucide-react";
...
export default function AdminApplications() {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: FormData }) => {
      const res = await fetch(`/api/applications/${id}`, { method: "PUT", body: data });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["applications"] });
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">Industry applications like Automotive, Packaging, Construction, etc.</p>
            </div>
          </div>
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  {editingItem ? "Update application details and image." : "Add a new product application with an image."}
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <Label htmlFor="image">Application Image</Label>
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-center py-8 text-gray-500">No applications yet. Add your first application!</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
